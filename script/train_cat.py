...
def training(train_bd, train_ed, q):
    ...
    model_dir = "../model/catboost/ens_e1000/%d_%d" % (train_bd_i, train_ed_i)
    ...
    for i in range(MODEL_NUM):
        ...
        if os.path.exists(model_name):
            ...
            train_catboost("%s_%s/%d"%(train_bd_i, train_ed_i, i), X_train, y_train, X_val, y_val)
...
def train_catboost(dir_path, X_train, y_train, X_val, y_val, i):
    ...
    if y_val is not None:
        ...
        error_rate = sqrt(mean_absolute_error(pred, y_val))
        print('Test error = {}'.format(error_rate))
    if not os.path.exists("../model/catboost/ens_e1000/%s"%dir_path):
        os.makedirs("../model/catboost/ens_e1000/%s"%dir_path)
    model.save_model("../model/catboost/ens_e1000/%s/model.pkl"%dir_path)
    ...
    for i in range(MODEL_NUM):
        ...
        if os.path.exists(model_name):
            ...
            if not os.path.exists("%s/%d" % (model_dir, i)):
                os.makedirs("%s/%d" % (model_dir, i))
            print("model[%d] training.." % (i+1))
            train_catboost("%s_%s/%d"%(train_bd_i, train_ed_i, i), X_train, y_train, X_val, y_val, i)
    print("Finish train model")
    q.put(scaler_x1)
    q.put(scaler_x2)
    q.put(scaler_x3)
...
def process_test(train_bd, train_ed, scaler, q):
    ...
    train_bd_i = int("%d%02d%02d" % (train_bd.year, train_bd.month, train_bd.day))
    train_ed_i = int("%d%02d%02d" % (train_ed.year, train_ed.month, train_ed.day))
    data_dir = "../data/catboost/ens_e1000_20110226_20170224"
    data_dir = "../data/catboost/ens_e1000"
    ...
    if len(X_test) == 0:
        ...
        pred = [0] * MODEL_NUM
        for i in range(MODEL_NUM):
            estimator = model.load_model("../model/catboost/ens_e1000/%d_%d/%d/model.pkl"%(train_bd_i, train_ed_i, i))
...
def simulation_weekly_train0(begin_date, end_date, delta_day=0, delta_year=0, courses=[0], kinds=[0], nData=47):
    ...
    while today >= begin_date:
        ...
        scaler_x6 = q.get()
        scaler_y = q.get()
        #scaler_x1, scaler_x2, scaler_x3, scaler_x4, scaler_x5, scaler_x6 = joblib.load('../model/catboost/ens_e1000/20110226_20170224/scaler_x.pkl')
        #scaler_y = joblib.load('../model/catboost/ens_e1000/20110226_20170224/scaler_y.pkl')
        q.put((sr, sscore))
...
if __name__ == '__main__':
    ...
    test_ed = datetime.date(2017, 10, 15)
