...
def load_estimators(model_dir):
    estimators = [0] * MODEL_NUM
    for i in range(MODEL_NUM):
        print("loading model[%d]..." % i)
        estimators[i] = joblib.load("../model/bagging_nn/ens_e1000/%s/%d/model.pkl"%(model_dir, i))
        estimators[i] = tfp.TensorflowRegressor('%s/%d' % (model_dir, i))
        estimators[i].load()
    return estimators
        ...
        for i in range(len(preds)+1):
            ...
            for idx, row in data.iterrows():
                ...
                if row['rcno'] != prev_rc or idx+1 == len(data):
                    ...
                    print_bet(rcdata, target=[[1,2],[1,2,3],[1,2,3]], total_bet=4000)
...
if __name__ == '__main__':
    ...
    for idx in range(1,2):
        ...
        model_dir = ["20110813_20170811", today_s][idx-1]
        date = init_date
        ...
        #scaler_x6 = q.get()
        #scaler_y = q.get()
        scaler_x1, scaler_x2, scaler_x3, scaler_x4, scaler_x5, scaler_x6 = joblib.load('../model/bagging_nn/ens_e1000/%s/scaler_xs.pkl' % (model_dir,))
        scaler_y = joblib.load('../model/bagging_nn/ens_e1000/%s/scaler_y.pkl' % (model_dir,))
        ...
        estimators = load_estimators(model_dir)
        if idx in [1,2,3,4]:
            fname = '../result/%d/%d_%d.txt' % (init_date/100%10000, date%100, idx)
