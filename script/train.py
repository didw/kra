...
def training(train_bd, train_ed, course=0, nData=47):
    ...
    if os.path.exists(model_name):
        ...
        X_train, Y_train, _, _ = get_data_from_csv(train_bd_i, train_ed_i, '../data/3_2007_2016.csv', course, nData=nData)
...
def simulation_weekly(begin_date, end_date, fname_result, delta_day=0, delta_year=0, course=0, kind=0, nData=47):
    ...
    while today <= end_date:
        ...
        if os.path.exists(model_name):
            ...
            print("Loading Datadata at %s - %s" % (str(train_bd), str(train_ed)))
            X_train, Y_train, _, _ = get_data_from_csv(train_bd_i, train_ed_i, '../data/3_2007_2016.csv', course, 0, nData=nData)
            print("%d data is fully loaded" % len(X_train))
            if len(X_train) < 10:
        ...
        X_test, Y_test, R_test, X_data = get_data_from_csv(test_bd_i, test_ed_i, '../data/3_2007_2016.csv', course, kind, nData=nData)
...
def simulation_weekly_train0(begin_date, end_date, delta_day=0, delta_year=0, courses=[0], kinds=[0], nData=47):
    ...
    while today <= end_date:
        ...
        if os.path.exists(model_name):
            ...
            print("Loading Datadata at %s - %s" % (str(train_bd), str(train_ed)))
            X_train, Y_train, _, _ = get_data_from_csv(train_bd_i, train_ed_i, '../data/3_2007_2016.csv', 0, nData=nData)
            print("%d data is fully loaded" % len(X_train))
            if len(X_train) < 10:
                res1, res2, res3, res4, res5, res6 = 0, 0, 0, 0, 0, 0
            else:
...
if __name__ == '__main__':
    ...
    for delta_year in [1,2,4]:
        for nData in [47, 69]:
            #simulation_weekly_train0(test_bd, test_ed, 0, delta_year, courses=[1000, 1200, 1300, 1400, 1500, 1600, 1800, 1900, 2000, 2200], nData=nData)
            for c in [1000, 1200, 1300, 1400, 1500, 1600, 1800, 1900, 2000, 2200, 0]:
                for k in [0]:
