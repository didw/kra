...
def get_hr_racescore(meet, hrno, _date, month, course, mode='File', md=mean_data()):
        ...
        for itemElm2 in itemElm.findAll('tr')[2:]:
            ...
            if racekind == '주' and len(race_sum[0]) == 0:
                ...
                if distance == 1000:
                    ...
                    race_sum[6].append(record * md.race_score[0][month_-1][humidity] / md.race_score[1600][month_-1][humidity])
                if course == distance:
    ...
    course_score[5] = np.mean(md.race_score[1600])
    ...
    if len(race_sum[6]) == 0:
        ...
        race_sum[6].reverse()
        for r in race_sum[6]:
            result[6] += 0.2 * (r - result[6])   # v1,v2: 0.5, v3: 0.2
    for i in range(len(race_sum)-1):
        if len(race_sum[i]) == 0:
            ...
            race_sum[i].reverse()
            for r in race_sum[i]:
                result[i] += 0.2 * (r - result[i])  # v1,v2: 0.5, v3: 0.2
