...
race_url = "http://race.kra.co.kr/dbdata/fileDownLoad.do?fn=chollian/seoul/girok/horse-rslt/"
...
while date > datetime.date(2011, 01, 01):
    ...
    request = "%s%ddacom21.rpt&meet=%d" % (race_url, date_s, meet)
    try:
        response_body = urlopen(request).read()
        fout = open("download/horse-rslt/horse-rslt_%d_%d.txt" % (meet, date_s), 'w')
