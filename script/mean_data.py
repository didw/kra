...
class mean_data:
    ...
    def update_race_score_qual(self, humidity, record):
        ...
        humidity -= 1
        try:
            self.race_score[300][humidity] += self.lr*10 * (record - self.race_score[300][humidity])
            self.race_score[300][20] = np.mean(self.race_score[300])
        except KeyError:
            return 0
    ...
    def update_data(self, df):
        ...
        for i in range(21):
            self.race_score[0][i] = (self.race_score[300][i] + self.race_score[400][i] + self.race_score[800][i] + self.race_score[900][i] + self.race_score[1000][i] + self.race_score[1200][i]) / 6
